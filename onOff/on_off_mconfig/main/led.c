...
static struct ledflag ledflag[KEY_MAX];
...
void set_led_flag(int id,onoff_led flag)
{
	...
	{
	case ON_LED:
		ledflag[id].led_flage = pdTRUE;/*点亮*/
		break;
	case OFF_LED:
		ledflag[id].led_flage = pdFALSE;
		break;
	default:
		break;
	}
...
void targiet_led(int id)
{
	if(ledflag[id].led_flage) {
		set_led_flag(id,OFF_LED);
	}else {
		set_led_flag(id,ON_LED);
	}
}

static void set_led(struct led led)
{
	if(led.flag) {
...
static void unset_led(struct led led)
{
	if(led.flag) {
		gpio_set_level(led.io_num,1);
	}else {
...
size_t get_flag_info(char * data, int count)
{
	...
	size = sprintf(data,"%d:{\"layer\":%d,\"Devs\":[{\"ID\":1,\"Flag\": \"%s\"},{\"ID\":2,\"Flag\": \"%s\"},{\"ID\":3,\"Flag\": \"%s\"},{\"ID\":4,\"Flag\": \"%s\"}]}",
						count,layer,ledflag[0].led_flage ? "On":"Off", 
