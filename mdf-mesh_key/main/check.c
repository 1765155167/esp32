...
mdf_err_t uart_decrypt(unsigned char * data, size_t * size)//解密
{
	mdf_err_t err;
	size_t len = *size;
	uint8_t id, ack_typ, typ;
	uint8_t crc;
	id = data[4];
	ack_typ = data[5];
	typ = data[6];
...
void uart_encryption(unsigned char * data, size_t * size)//加密
{
	uint8_t crc;
	uint8_t id = 1, ack_typ = 1, typ = 1;
	size_t len = *size;
	for(int i = len; i >= 0; i--)
	{
		data[i + 7] = data[i];
