...
void root_task(void * arg)
{
	...
	for(;;)
	{
		...
		mesh_data.size = strlen(c_data);
		for (int i = 0; i < route_table_size; i++) {
			err = esp_mesh_send(&route_table[i], &mesh_data, MESH_DATA_P2P, NULL, 0);
		}
		// /*读数据*/
		// err = esp_mesh_recv(&from, &mesh_data, timeout_ms, &flag, NULL, 0);
		// if(err == ESP_OK)
		// {
		...
		// 	}
		// 	err = esp_mesh_get_routing_table((mesh_addr_t *) &route_table, 5 * 6, &route_table_size);
		// 	router_err(err);
		// 	/* 向所有子设备发送信息 */
		// 	for (int i = 0; i < route_table_size; i++) 
		// 	{
		...
		// 		if(err == ESP_OK)
		// 		{
		// 			ESP_LOGI(MESH_TAG,"root esp_mesh_send ok");
		// 		}
		// 	}
...
esp_err_t mymesh_init(void)
{
    ...
    sta_ip.ip.addr = ipaddr_addr(IPADDRESS);
...
{
	const char *MESH_TAG = "mesh_read";
	switch (err)
	{
		case ESP_OK:
			ESP_LOGI(MESH_TAG,"ESP_OK");
			break;
		case ESP_ERR_MESH_ARGUMENT:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_ARGUMENT");
			break;
		case ESP_ERR_MESH_NOT_START:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_NOT_START");
			break;
		case ESP_ERR_MESH_TIMEOUT:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_TIMEOUT");
			break;
		case ESP_ERR_MESH_DISCARD:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_DISCARD");
			break;
		default:
			break;
	}
...
{
	const char *MESH_TAG = "mesh_write";
	switch (err)
	{
		case ESP_OK:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_DISCARD");
		...
		case ESP_ERR_MESH_DISCONNECTED:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_DISCONNECTED");
			break;
		case ESP_ERR_MESH_EXCEED_MTU:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_EXCEED_MTU");
			break;
		case ESP_ERR_MESH_NO_MEMORY:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_NO_MEMORY");
			break;
		case ESP_ERR_MESH_TIMEOUT:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_TIMEOUT");
			break;
		case ESP_ERR_MESH_QUEUE_FULL:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_QUEUE_FULL");
			break;
		case ESP_ERR_MESH_NO_ROUTE_FOUND:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_NO_ROUTE_FOUND");
			break;
		case ESP_ERR_MESH_DISCARD:
			ESP_LOGI(MESH_TAG,"ESP_ERR_MESH_DISCARD");
			break;
		default:
			ESP_LOGI(MESH_TAG,"mesh_write default");
			break;
	}
}
...
{
	const char *MESH_TAG = "router_err";
	switch (err)
	{
		case ESP_OK:
			ESP_LOGI(MESH_TAG,"获取路由表信息 ok ");
			break;
		case ESP_ERR_MESH_ARGUMENT:
			ESP_LOGI(MESH_TAG,"获取路由表信息 ESP_ERR_MESH_ARGUMENT");
			break;
		default:
			break;
	}
